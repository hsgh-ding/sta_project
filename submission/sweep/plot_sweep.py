#!/usr/bin/env python3
"""Plot HBT capacitance sweep results — GCD (dual Y-axis)."""
import csv, sys, os

def main():
    csv_path = sys.argv[1] if len(sys.argv) > 1 else "hbt_sweep.csv"
    ...
    with open(csv_path) as f:
        for row in csv.DictReader(f):
            c_vals.append(float(row["hbt_c"]))
            setup_wns.append(float(row["setup_wns"]))
            hold_wns.append(float(row["hold_wns"]))
    ...
    ax1.set_title("GCD — HBT Capacitance vs. WNS (ss_ff corner)", fontsize=14)
